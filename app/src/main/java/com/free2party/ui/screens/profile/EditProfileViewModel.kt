...
class EditProfileViewModel @Inject constructor(
    ...
    var uiState by mutableStateOf<EditProfileUiState>(EditProfileUiState.Loading)
        internal set
    ...
    fun isBirthdayValid(context: Context): Boolean {
        val pattern = (uiState as? EditProfileUiState.Success)?.user?.settings?.datePattern
    ...
    val hasChanges by derivedStateOf {
        val user = (uiState as? EditProfileUiState.Success)?.user ?: return@derivedStateOf false
        ...
        viewModelScope.launch {
            userRepository.observeUser(userRepository.currentUserId)
                .catch { e ->
                    ...
                    uiState =
                        EditProfileUiState.Error(mapToUiText(e, R.string.error_profile_not_found))
                }
                .collect { user ->
                    ...
                    if (currentState !is EditProfileUiState.Success) {
                        ...
                        uiState =
                            EditProfileUiState.Success(user = user)
                    } else {
    ...
    fun discardChanges() {
        val user = (uiState as? EditProfileUiState.Success)?.user ?: return
        initializeFields(user)
    }

    fun updateProfile(context: Context) {
        val currentState = uiState as? EditProfileUiState.Success ?: return
        ...
        if (firstName.isBlank() || lastName.isBlank()) {
            uiState =
                EditProfileUiState.Error(UiText.StringResource(R.string.error_required_fields))
            return
        }

        if (!isPhoneNumberValid) {
            uiState =
                EditProfileUiState.Error(UiText.StringResource(R.string.error_invalid_phone))
            return
        }

        if (!isBirthdayValid(context)) {
            uiState = EditProfileUiState.Error(UiText.StringResource(R.string.error_invalid_date))
            return
        }

        if (!isWhatsappNumberValid) {
            uiState =
                EditProfileUiState.Error(UiText.StringResource(R.string.error_invalid_whatsapp))
            return
        }
        ...
        val nameChanged =
            firstName != currentState.user.firstName || lastName != currentState.user.lastName
                ...
                .onSuccess { downloadUrl ->
                    ...
                    userRepository.updateUser(updatedUser)
                        .onSuccess {
                            ...
                            uiState =
                                (uiState as? EditProfileUiState.Success)?.copy(isUploadingImage = false)
                                    ?: uiState
                            _uiEvent.emit(EditProfileUiEvent.ShowToast(UiText.StringResource(R.string.toast_profile_picture_updated)))
                        }
                        .onFailure { e ->
                            uiState =
                                (uiState as? EditProfileUiState.Success)?.copy(isUploadingImage = false)
    ...
    fun deleteAccount() {
        ...
        viewModelScope.launch {
            userRepository.deleteAccount()
                .onSuccess {
                    _uiEvent.emit(EditProfileUiEvent.AccountDeleted)
                }
                .onFailure { e ->
                    uiState =
                        (uiState as? EditProfileUiState.Success)?.copy(isSaving = false) ?: uiState
